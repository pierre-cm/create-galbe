...
var package_default = {
  ...
  author: {
    name: "Pierre Caillaud M",
    url: "https://github.com/pierre-cm"
  },
  repository: {
    type: "git",
    url: "https://github.com/pierre-cm/create-galbe"
  },
  license: "MIT",
  homepage: "https://galbe.dev",
  scripts: {
    build: "bun build ./index.ts --outfile index.js --target bun",
    publish: "bun run build && npm publish"
  },
  devDependencies: {
    "@swc/core": "^1.5.29-nightly-20240612.2",
    "@types/bun": "latest",
    "@types/prompts": "^2.4.9",
    commander: "^12.0.0",
    typescript: "^5.0.0"
...
program.name("galbe").description("CLI to setup a galbe project or plugin").version(package_default.version).argument("[path]", "project path").option("-t, --template <string>", "template name").option(`-l, --lang <string>", "language (${languages})`).action(async (arg, opt) => {
  let [path, { template, lang }] = [arg, opt];
  if (!(template in templates)) {
    template = (await import_prompts.default({
      type: "select",
      name: "template",
      message: "Select a template",
      choices: Object.entries(templates).map(([name2, { description }]) => ({
        title: name2,
        description,
        value: name2,
        short: name2
  ...
  if (!(lang in languages)) {
    lang = (await import_prompts.default({
      type: "select",
      name: "lang",
      message: "Select a language",
      choices: Object.entries(languages).map(([short, { name: name2 }]) => ({
        title: name2,
        value: short,
        short
      }))
  ...
  let defaultName = templates[template].defaultName;
  if (!path)
    path = `./${defaultName}`;
  let name = path === "." ? defaultName : path?.match(/\/?([^/]+)$/)?.[1] || defaultName;
  ...
  let templateDir = `${import.meta.dir}/templates/${template}`;
  await cp(`${templateDir}/${lang}`, path, {
    recursive: true
  });
  if (existsSync(`${import.meta.dir}/scripts/${template}.ts`))
    await (await import(`${import.meta.dir}/scripts/${template}.ts`))?.default(templateDir, path, lang);
